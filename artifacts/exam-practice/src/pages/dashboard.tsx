...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div>
        <h1 className="font-serif text-4xl text-foreground">Welcome back, {dashboard.user.username}.</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="bg-card border-card-border shadow-sm">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground uppercase tracking-wider">Real Test Passes</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="flex items-baseline gap-2">
              <span className="text-4xl font-serif text-foreground">{dashboard.realTestStat.passes}</span>
              <span className="text-sm text-muted-foreground">/ {dashboard.realTestStat.attempts} attempts</span>
            </div>
            {dashboard.realTestStat.bestScaled > 0 && (
              <p className="text-xs text-success mt-1">Best score: {dashboard.realTestStat.bestScaled}</p>
            )}
          </CardContent>
        </Card>
        ...
        <Card className="bg-card border-card-border shadow-sm">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground uppercase tracking-wider">To Review</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="flex gap-6">
              <div>
                <div className="flex items-baseline gap-1">
                  ...
                  <span className="text-2xl font-serif">{dashboard.wrongAnswerCount}</span>
                </div>
              ...
              <div>
                <div className="flex items-baseline gap-1">
                  ...
                  <span className="text-2xl font-serif">{dashboard.bookmarkCount}</span>
                </div>
                      ...
                      {attempt.submittedAt ? (
                        <>
                          ...
                          {attempt.isRealTest && attempt.passed !== null && (
                            <div className={`text-xs font-medium ${attempt.passed ? 'text-success' : 'text-destructive'}`}>
